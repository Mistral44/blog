...
require get_template_directory() . '/inc/class-wpajax-recent-post-widget.php';
require get_template_directory() . '/inc/class-wpajax-subscribe-form-widget.php';

function wpajax_register_widget() {
	register_widget( 'Wpajax_Widget_Recent_Posts' );
	register_widget( 'Wpajax_Widget_Subscribe' );
}
add_action( 'widgets_init', 'wpajax_register_widget' );


function wpajax_setup() {
 
	load_theme_textdomain('wpajax', get_template_directory(). '/lang');
	...
	add_image_size( 'wpajax-recent-post', 80, 80, true ); 
...
add_action('after_setup_theme', 'wpajax_setup');

function wpajax_scripts() {
	...
	wp_enqueue_style ( 'font-open-sans', 'https://fonts.googleapis.com/css?family=Open+Sans:400,600,7000' );
...
add_action( 'wp_enqueue_scripts', 'wpajax_scripts');
...
function wpajax_the_breadcrumb(){
	global $post;
	if(!is_home()){ 
...
function wpajax_pagination( $args = array() ) {
...
function wpajax_customize_register( $wp_customize ) {
	$wp_customize->add_setting( 'header_social' , array(
		'default'   => __('Share Your Favorite Mobile Apps With Your Friends', 'wpajax'),
		'transport' => 'refresh',
	));

	$wp_customize->add_setting( 'facebook_social' , array(
		'default'   => __('Url facebook', 'wpajax'),
		'transport' => 'refresh',
	));

	$wp_customize->add_setting( 'twitter_social' , array(
		'default'   => __('Url twitter', 'wpajax'),
		'transport' => 'refresh',
	));

	$wp_customize->add_setting( 'linkedin_social' , array(
		'default'   => __('Url linkedin', 'wpajax'),
		'transport' => 'refresh',
	));

	$wp_customize->add_setting( 'google_social' , array(
		'default'   => __('Url google', 'wpajax'),
		'transport' => 'refresh',
	));

	$wp_customize->add_setting( 'youtube_social' , array(
		'default'   => __('Url youtube', 'wpajax'),
		'transport' => 'refresh',
	));

	$wp_customize->add_setting( 'instagram_social' , array(
		'default'   => __('Url instagram', 'wpajax'),
		'transport' => 'refresh',
	));

	$wp_customize->add_setting( 'footer_copy' , array(
		'default'   => __('Copyright text', 'wpajax'),
		'transport' => 'refresh',
	));

	$wp_customize->add_section( 'social_section' , array(
		'title'      => __( 'Social settings', 'wpajax' ),
		'priority'   => 30,
	));

	$wp_customize->add_section( 'footer_section' , array(
		'title'      => __( 'Footer settings', 'wpajax' ),
		'priority'   => 31,
	));
	...
	$wp_customize->add_control(
		'header_social', 
		array(
			'label'    => __( 'Social header in footer', 'wpajax' ),
	...
	$wp_customize->add_control(
		'facebook_social', 
		array(
			'label'    => __( 'Facebook profile url', 'wpajax' ),
			'section'  => 'social_section',
	...
	$wp_customize->add_control(
		'twitter_social', 
		array(
			'label'    => __( 'Twitter profile url', 'wpajax' ),
			'section'  => 'social_section',
	...
	$wp_customize->add_control(
		'linkedin_social', 
		array(
			'label'    => __( 'Linkedin profile url', 'wpajax' ),
			'section'  => 'social_section',
	...
	$wp_customize->add_control(
		'google_social', 
		array(
			'label'    => __( 'Google profile url', 'wpajax' ),
			'section'  => 'social_section',
	...
	$wp_customize->add_control(
		'youtube_social', 
		array(
			'label'    => __( 'Youtube profile url', 'wpajax' ),
			'section'  => 'social_section',
	...
	$wp_customize->add_control(
		'instagram_social', 
		array(
			'label'    => __( 'Instagram profile url', 'wpajax' ),
			'section'  => 'social_section',
	...
	$wp_customize->add_control(
		'footer_copy', 
		array(
			'label'    => __( 'Footer copyright', 'wpajax' ),
...
add_action( 'customize_register', 'wpajax_customize_register' );

/**
 * Add a sidebar.
 */
function wpajax_widgets_init() {
    register_sidebar( array(
        'name'          => __( 'Main Sidebar', 'wpajax' ),
        'id'            => 'sidebar-1',
        'description'   => __( 'Widgets in this area will be shown on all posts and pages.', 'wpajax' ),
...
add_action( 'widgets_init', 'wpajax_widgets_init' );


//======================================ВИДЖЕТ КАТЕГОРИЙ==========================================================
function wpajax_widget_categories($args){

	$walker = new Walker_Categories_Wpajax();
	$args = array_merge($args, array('walker' => $walker));

	return $args;
}
add_filter('widget_categories_args', 'wpajax_widget_categories');

class Walker_Categories_Wpajax extends Walker_Category {
	/**
...
// =======================================ВИДЖЕТ ОБЛАКА ТЕГОВ==============================================
function wpajax_tag_cloud($args) {
	$args['format'] = 'list';
	$args['smallest'] = 14;
	$args['unit'] = 'px';
	return $args;
...
add_filter('widget_tag_cloud_args', 'wpajax_tag_cloud');

// Change order comment fields
add_filter('comment_form_fields', 'wpajax_reorder_comment_fields' );
function wpajax_reorder_comment_fields( $fields ){
    $new_fields = array(); 
...
function wpajax_list_comment($comment, $args, $depth) {
    if ( 'div' === $args['style'] ) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
